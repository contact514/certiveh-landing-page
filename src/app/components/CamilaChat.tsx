    ...
    try {
      const res = await fetch(SUPPORT_CHAT_URL, {
        ...
        body: JSON.stringify({
          ...
          screenContext: 'Landing page de certiveh.co — usuario no registrado, posible interesado en el servicio.',
        }),
      });
        ...
        >
          ...
          <div style={{
            ...
            {messages.length === 0 && !isTyping && (
              <div style={{ textAlign: 'center', color: '#94a3b8', fontSize: 13, padding: '40px 16px' }}>
                Escríbele a Camila cualquier duda sobre CertiVeh
              </div>
            )}
            {messages.map(m => (
              <div key={m.id} style={{
